...
static void prv_handle_connection_event(struct ble_gap_event *event) {
  ...
  if (ble_gap_conn_find(event->connect.conn_handle, &desc) != 0) {
    PBL_LOG(LOG_LEVEL_ERROR, "Failed to find connection descriptor");
    return;
  }
  ...
  struct BleConnectionCompleteEvent complete_event = {
      ...
      .status = HciStatusCode_Success,  // TODO: can this be something else?
      // .irk = {0},
      .is_resolved = false,  // TODO: check if this is right
  };
  ...
  PBL_LOG(LOG_LEVEL_INFO, "device connected! connection handle:%d addr:" BT_DEVICE_ADDRESS_FMT,
...
static void prv_handle_disconnection_event(struct ble_gap_event *event) {
  ...
  PBL_LOG(LOG_LEVEL_INFO, "device disconnected!");
}
...
static void prv_handle_identity_resolved_event(struct ble_gap_event *event) {
  ...
  PBL_LOG(LOG_LEVEL_INFO, "identity resolved, old addr:" BT_DEVICE_ADDRESS_FMT,
          BT_DEVICE_ADDRESS_XPLODE(addr_change_event.device.address));
  PBL_LOG(LOG_LEVEL_INFO, "identity resolved, new addr:" BT_DEVICE_ADDRESS_FMT,
...
static void prv_handle_subscription_event(struct ble_gap_event *event) {
  PBL_LOG(LOG_LEVEL_INFO, "connhandle: %d attr:%d notify:%d/%d indicate:%d/%d",
          event->subscribe.conn_handle, event->subscribe.attr_handle, event->subscribe.prev_notify,
          event->subscribe.cur_notify, event->subscribe.prev_indicate,
          event->subscribe.cur_indicate);

  pebble_pairing_service_get_connectivity_send_notification(event->subscribe.conn_handle,
...
static int prv_handle_gap_event(struct ble_gap_event *event, void *arg) {
  switch (event->type) {
    case BLE_GAP_EVENT_CONNECT:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_CONNECT");
      prv_handle_connection_event(event);
      break;
    case BLE_GAP_EVENT_DISCONNECT:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_DISCONNECT");
      prv_handle_disconnection_event(event);
      break;
    case BLE_GAP_EVENT_ENC_CHANGE:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_ENC_CHANGE");
      prv_handle_enc_change_event(event);
      break;
    case BLE_GAP_EVENT_CONN_UPDATE:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_CONN_UPDATE");
      prv_handle_conn_params_updated_event(event);
      break;
    case BLE_GAP_EVENT_PASSKEY_ACTION:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_PASSKEY_ACTION");
      prv_handle_passkey_event(event);
      break;
    case BLE_GAP_EVENT_IDENTITY_RESOLVED:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_IDENTITY_RESOLVED");
      prv_handle_identity_resolved_event(event);
      break;
    case BLE_GAP_EVENT_PAIRING_COMPLETE:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_PAIRING_COMPLETE");
      prv_handle_pairing_complete_event(event);
      break;
    case BLE_GAP_EVENT_MTU:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_MTU");
      prv_handle_mtu_change_event(event);
      break;
    case BLE_GAP_EVENT_SUBSCRIBE:
      PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_SUBSCRIBE");
      prv_handle_subscription_event(event);
      break;
    case BLE_GAP_EVENT_NOTIFY_RX:
      // PBL_LOG(LOG_LEVEL_INFO, "BLE_GAP_EVENT_NOTIFY_RX");
      prv_handle_notification_rx_event(event);
      break;
    default:
      PBL_LOG(LOG_LEVEL_INFO, "Unhandled GAP event: %d", event->type);
      break;
  }
  return 0;
}
