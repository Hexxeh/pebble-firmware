...
static bool prv_convert_service_and_notify_os_cb(ListNode *node, void *context) {
  ...
  PBL_LOG(LOG_LEVEL_INFO, "bt_driver_cb_gatt_client_discovery_handle_indication for service %s",
          service_uuid_str);
...
static void prv_discover_next_dscs(uint16_t conn_handle, GATTServiceDiscoveryContext *context) {
  ...
  PBL_LOG(LOG_LEVEL_INFO, "discovery: ble_gattc_disc_all_dscs (%d -> %d) returned %d", start_handle,
          end_handle, rc);
}
...
static void prv_discover_next_chrs(uint16_t conn_handle, GATTServiceDiscoveryContext *context) {
  ...
  PBL_LOG(LOG_LEVEL_INFO, "discovery: ble_gattc_disc_all_chrs returned %d", rc);
}
...
static int prv_find_dsc_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
  ...
  switch (error->status) {
    case 0:
      ...
      PBL_LOG(LOG_LEVEL_INFO, "discovery: found descriptor %s", chr_uuid_str);
      ...
      break;
    case BLE_HS_EDONE:
      PBL_LOG(LOG_LEVEL_INFO, "discovery: descriptor discovery done");
    ...
    default:
      PBL_LOG(LOG_LEVEL_INFO, "discovery: descriptor discovery error: %d", error->status);
...
static int prv_find_chr_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
  ...
  switch (error->status) {
    case 0:
      ...
      PBL_LOG(LOG_LEVEL_INFO, "discovery: found characteristic %s (val hdl: %d, def hdl: %d)",
    ...
    case BLE_HS_EDONE:
      PBL_LOG(LOG_LEVEL_INFO, "discovery: characteristic discovery done");
    ...
    default:
      PBL_LOG(LOG_LEVEL_INFO, "discovery: characteristic discovery error: %d", error->status);
      bt_driver_cb_gatt_client_discovery_complete(context->connection,
                                                  BTErrnoInternalErrorBegin + error->status);
      prv_free_discovery_context(context);
      break;
...
static int prv_find_inc_svc_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
  ...
  switch (error->status) {
    case 0:
      ...
      PBL_LOG(LOG_LEVEL_INFO, "discovery: found service %s, %d-%d (total %lu)", service_uuid_str,
              service->start_handle, service->end_handle, list_count(context->services));
      break;

    case BLE_HS_EDONE:
      PBL_LOG(LOG_LEVEL_INFO, "service discovery complete");
      ...
      if (context->services != NULL) {
        ...
        PBL_LOG(LOG_LEVEL_INFO,
                "discovery: bt_driver_cb_gatt_client_discovery_complete returned %d", completed);
        prv_free_discovery_context(context);
      }
    ...
    default:
      PBL_LOG(LOG_LEVEL_INFO, "service discovery error: %d", error->status);
      bt_driver_cb_gatt_client_discovery_complete(context->connection,
                                                  BTErrnoInternalErrorBegin + error->status);
      prv_free_discovery_context(context);
      break;
...
BTErrno bt_driver_gatt_start_discovery_range(const GAPLEConnection *connection,
  ...
  PBL_LOG(LOG_LEVEL_INFO, "bt_driver_gatt_start_discovery_range %d-%d", data->start, data->end);
  ...
  // int rc = ble_gattc_find_inc_svcs(conn_handle, data->start, data->end, prv_find_inc_svc_cb,
  //                                  (void *)context);
